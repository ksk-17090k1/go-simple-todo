...
[`net/http`](https://pkg.go.dev/net/http) のサーバサイドで押さえるべき型はたった 3 つです。

### 2.1.1 [`http.Handler`](https://pkg.go.dev/net/http#Handler) インターフェイス
...
### 2.1.2 [`http.HandlerFunc`](https://pkg.go.dev/net/http#HandlerFunc)
...
### 2.1.3 [`http.ServeMux`](https://pkg.go.dev/net/http#ServeMux)
...
明示的に新しい `ServeMux` を作っています。「`nil` を渡すとデフォルトの [`DefaultServeMux`](https://pkg.go.dev/net/http#DefaultServeMux) が使われる」という慣習もありますが、
...
### [`mux.HandleFunc("GET /hello", ...)`](https://pkg.go.dev/net/http#ServeMux.HandleFunc)
...
### [`http.ListenAndServe(":8080", mux)`](https://pkg.go.dev/net/http#ListenAndServe)
...
[`http.ResponseWriter`](https://pkg.go.dev/net/http#ResponseWriter) は次のように使います。
